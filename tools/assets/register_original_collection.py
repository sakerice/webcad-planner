...
for source in ['original-collection.json','exterior-collection.json','entry-storage-collection.json']:
...
for item in items:
 ...
 j['asset']['extras']=dict(j['asset'].get('extras',{}),front='+Z',up='+Y',provenance='Independently authored in Blender; no imported geometry or imagery',source='tools/blender/build_entry_storage.py' if item.get('mirrorOption') else 'tools/blender/build_exterior_details.py' if item.get('group')=='外構' else 'tools/blender/build_original_furniture.py')
