...
def apply_review_26_joints(plan):
    """受領版26に残った「通り芯からのずれ」だけを戻す。間取りは変えない。

    ユーザーの手直しは残す。ここで触るのは、**アプリが表示のために黙って
    吸着させていて、保存されている数値だけがずれている**2か所だけ。

      壁1247   始端が壁1002の芯から20mmずれ、交差部に段差が出る
      開口1248 アーチ開口の x/y/rot が、どの壁線上にも無い。アプリは
               getOpeningWallInfo() で最寄りの壁1014へ20mm吸着させて
               描いているので、**見た目は変わらない。** 保存値を
               その位置に合わせて、検査とアプリの見方を一致させる。

    ずれたままだと、検査が毎回3件挙げ続けて本当の指摘が埋もれる。
    """
    walls = {w['id']: w for w in plan['walls']}
    items = {i['id']: i for i in plan['items']}
    walls[1247]['y1'] = -600
    # 保存値は rot=0(幅900が東西方向)だったが、付いている壁1014は南北方向。
    # **アプリは getOpeningWallInfo() で rot=90 に読み替えて描いている**ので
    # 見た目は変わらない。保存値を描いている向きに合わせる。
    # x/y は**左上の角**であって中心ではない。中心 (x+w/2, y+d/2) を
    # 壁1014の芯 (7280, -70) に置く。
    items[1248].update(x=7280 - items[1248]['w'] / 2,
                       y=-70 - items[1248]['d'] / 2, rot=90)
    return plan
