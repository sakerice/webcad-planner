...
test('configured furniture cannot bypass facing/material correction when deselected',()=>{
 const sandbox={ren:{},ST:{selected:null},GLTF_MODEL_CONFIG:{bed:{rotY:Math.PI}}};
...
test('existing furniture preserves physical facing and migration is idempotent',()=>{
 ...
 const sandbox={DATA:{items:[item]},ST:{selected:null},getFmpItem:()=>({category:'ソファ'}),bestFmpType:t=>t};
...
test('car migration preserves old placement while new car assets use +Z front',()=>{
 ...
 const item={type:'car',rot:0};const s={DATA:{items:[item]},ST:{selected:null},getFmpItem:()=>null,bestFmpType:t=>t};
