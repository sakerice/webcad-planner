...
test('new exterior fixtures sit at ground datum, independently of indoor floor elevation',()=>{
 ...
 const base=fn('item3DBaseY',{isGroundLevelItemType:ground,isContextExteriorItemType:()=>false,isFloorAwareGroundItemType:()=>false,roomFloorAt:()=>.15});assert.equal(base({type:'original-mailbox',floor:1}),0);assert.equal(base({type:'original-sofa',floor:1}),.15);
});
