...
try{const p=await b.newPage({viewport:{width:1000,height:700}});const errors=[];p.on('pageerror',e=>errors.push(e.message));p.on('console',m=>{if(m.type()==='error')errors.push(m.text());});await p.goto('http://localhost:8932/');await p.waitForFunction(()=>window.THREE);await p.evaluate(()=>init3D());await p.waitForFunction(()=>_skyPhotoCache.common);
