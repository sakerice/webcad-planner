...
paint=mat('Azure metallic paint',(.012,.24,.42),.32,.42);paint.node_tree.nodes['Principled BSDF'].inputs['Coat Weight'].default_value=.5
...
S=[(-2.05,.68,.22,.82),(-1.96,.81,.15,.91),(-1.62,.875,.14,.96),(-1.15,.88,.14,.94),(-.4,.885,.14,.92),(.45,.88,.14,.90),(1.1,.86,.15,.85),(1.65,.84,.16,.80),(1.98,.80,.18,.76),(2.05,.76,.20,.73)]
v=[];f=[]
for y,w,z0,z1 in S:
 for j in range(32):
...
for s in (-1,1):
 side=[(s*.80,.88,.88),(s*.68,.22,1.39),(s*.66,-1.06,1.41),(s*.79,-1.62,.96)]
 q=panel('Glazing side',side,glass,.018,axis=0,sign=s)
 line('Window weatherseal',side+[side[0]],black,.012)
 line('A pillar',[side[0],side[1]],paint,.035)
 line('Roof rail',[side[1],side[2]],paint,.032)
 line('C pillar',[side[2],side[3]],paint,.045)
 line('Belt moulding',[side[3],side[0]],paint,.038)
 line('B pillar',[(s*.785,-.40,.89),(s*.673,-.40,1.41)],black,.032)
...
for n,pts in [('Windscreen',[(-.80,.88,.88),(.80,.88,.88),(.68,.22,1.39),(-.68,.22,1.39)]),('Rear window',[(-.66,-1.06,1.41),(.66,-1.06,1.41),(.79,-1.62,.96),(-.79,-1.62,.96)])]:
...
# Cabin surfaces and upholstered seating; no external mesh inputs.
seatmat=mat('Original woven charcoal upholstery',(.055,.065,.074),.92)
glass.node_tree.nodes['Principled BSDF'].inputs['Alpha'].default_value=.68
for x in (-.40,.40):
 box('Seat cushion',(x,-.15,.51),(.53,.55,.14),seatmat,.06)
 ob=box('Seat back',(x,-.39,.83),(.51,.14,.58),seatmat,.065);ob.rotation_euler.x=-.10
 box('Head restraint',(x,-.43,1.17),(.29,.12,.20),seatmat,.055)
box('Rear bench',(0,-1.05,.58),(1.28,.40,.18),seatmat,.06)
box('Rear backrest',(0,-1.29,.83),(1.26,.14,.50),seatmat,.065)
box('Cabin floor',(0,-.30,.32),(1.50,2.3,.07),black,.02)
box('Dashboard',(0,.55,.83),(1.48,.34,.15),black,.06)
box('Centre console',(0,-.02,.60),(.19,.65,.18),black,.04)
bpy.ops.mesh.primitive_torus_add(major_segments=48,minor_segments=12,major_radius=.145,minor_radius=.017,location=(.4,.34,.96),rotation=(math.pi/2+.25,0,0));bpy.context.object.name='Steering rim';bpy.context.object.data.materials.append(black)
box('Steering hub',(.4,.34,.96),(.10,.04,.065),black,.025)
# Integrate the optical housings and lower intake into the shell.
body=bpy.data.objects.get('Continuous formed body')
from mathutils.bvhtree import BVHTree
front_surface=BVHTree.FromObject(body,bpy.context.evaluated_depsgraph_get())
def recess(name,pos,dims):
 cutter=box(name,pos,dims,black,.03);bpy.context.view_layer.objects.active=cutter
 for mod in list(cutter.modifiers):bpy.ops.object.modifier_apply(modifier=mod.name)
 mod=body.modifiers.new(name,'BOOLEAN');mod.operation='DIFFERENCE';mod.object=cutter;bpy.context.view_layer.objects.active=body;bpy.ops.object.modifier_apply(modifier=mod.name);bpy.data.objects.remove(cutter,do_unlink=True)
recess('Integrated intake cut',(0,2.0,.40),(1.10,.40,.23))
for ob in list(bpy.context.scene.objects):
 if ob.name.startswith(('Headlamp','LED signature','Rear optical cluster','Tail light guide')):bpy.data.objects.remove(ob,do_unlink=True)
for sign in (-1,1):
 recess('Integrated lamp cut',(sign*.52,2.0,.675),(.40,.35,.085))
 box('Inset projector housing',(sign*.52,2.014,.675),(.38,.035,.065),dark,.015)
 for k in range(3):
  box('Projector optics',(sign*(.40+k*.105),2.036,.675),(.066,.010,.036),alloy,.012)
 line('LED eyebrow',[(sign*.34,2.037,.698),(sign*.66,2.037,.698)],white,.006)
 box('Rear lamp housing',(sign*.55,-1.94,.83),(.38,.035,.070),dark,.022)
 line('Rear light ribbon',[(sign*.35,-1.966,.85),(sign*.72,-1.966,.85),(sign*.73,-1.95,.82)],red,.009)
# Pressed wheel-arch returns and integrated side sills.
for sign in (-1,1):
 for ay in (-1.28,1.24):
  points=[]
  for j in range(65):
   a=math.pi*j/64;points.append((sign*.856,ay+.354*math.cos(a),.33+.354*math.sin(a)))
  line('Pressed fender return',points,paint,.012)
 box('Painted rocker panel',(sign*.853,-.02,.20),(.07,1.78,.09),paint,.025)
# Lamp lens surfaces follow the front corner radius; remove projecting blocks.
for ob in list(bpy.context.scene.objects):
 if ob.name.startswith(('Inset projector housing','Projector optics','LED eyebrow')):bpy.data.objects.remove(ob,do_unlink=True)
for sign in (-1,1):
 pts=[(sign*.32,2.041,.706),(sign*.73,1.99,.721),(sign*.72,1.995,.640),(sign*.32,2.041,.658)]
 panel('Flush headlamp lens',pts,dark,.002,axis=1)
 line('Continuous LED contour',[pts[0],pts[1],pts[2]],white,.007)
 for k in range(3):
  x=.40+k*.105;y=2.046-(x-.32)*.125
  line('Projector light guide',[(sign*x,y,.680),(sign*(x+.046),y-.004,.683)],alloy,.008)
# Position every optical vertex on the uncut body surface, with a 2mm lens offset.
for ob in bpy.context.scene.objects:
 if not ob.name.startswith(('Flush headlamp lens','Continuous LED contour','Projector light guide')):continue
 if ob.type=='MESH':
  for vertex in ob.data.vertices:
   p=vertex.co;hit=front_surface.ray_cast(Vector((p.x,3,p.z)),Vector((0,-1,0)))
   if hit[0] is not None:p.y=hit[0].y+.002
 elif ob.type=='CURVE':
  for spline in ob.data.splines:
   for point in spline.points:
    p=point.co;hit=front_surface.ray_cast(Vector((p.x,3,p.z)),Vector((0,-1,0)))
    if hit[0] is not None:p.y=hit[0].y+.005
...
bpy.ops.wm.save_as_mainfile(filepath=str(out/'original_hatchback_v2.blend'))
bpy.ops.export_scene.gltf(filepath=str(out/'original_hatchback_v2.glb'),export_format='GLB',export_apply=True)
...
sc=bpy.context.scene;sc.camera=c;sc.render.engine='CYCLES';sc.cycles.samples=32;sc.render.resolution_x=1100;sc.render.resolution_y=800;sc.render.resolution_percentage=100;sc.render.filepath=str(out/'original_hatchback_v2.png');bpy.ops.render.render(write_still=True)
