#!/usr/bin/env python3
"""光を取り込む2階LDKを中心にした都市型3階建て・3LDK。

建物外形5460×8190mm、南道路、敷地約83㎡。1階に主寝室と水まわり、
2階に全幅14.91㎡の吹抜LDK、3階に同面積の子ども室2室と家具を置かないギャラリー。
末尾のレビュー適用で、ユーザーが削除・移動した配置を保持する。
家具の正面は設計上0=北、90=東で記述し、出力時に
...
room("階段", 4550, 0, 910, 3640, 1, texture="wood_oak")
room("ランドリー", 0, 1820, 1820, 1820, 1, texture="tile_floor")
room("納戸", 1820, 1820, 910, 1820, 1, texture="wood_oak")
room("廊下", 2730, 1820, 1820, 1820, 1, texture="wood_oak")
room("主寝室", 0, 3640, 2730, 4550, 1, texture="wood_oak")
room("ホール", 2730, 3640, 2730, 1820, 1, texture="wood_oak")
room("クローゼット", 2730, 5460, 910, 2730, 1, texture="wood_oak")
...
door("door-fold-w", 2730, 6900, 1650, 1, vertical=True)          # クローゼット(玄関から)
door("door-opening", 4095, 5460, 910, 1)                      # 玄関→ホール(上り框)
door("door-front", 4800, BD, 940, 1, color=COL_DOOR)          # 玄関ドア

# 窓は上端2030の通りで揃える
win(910, 0, 1, "06905", 2100, 450, kind="fix")        # 浴室北(採光FIX)
win(2730, 0, 1, "06905", 2100, 450, kind="fix")       # 洗面北(採光FIX)
...
wall(0, 2730, 2730, 2730, 2)
wall(2730, 2730, 3640, 2730, 2)
# 南側は建物の全幅をリビングに開放する。         # LDK|LDK東 (直下 1F x3640)
       # LDK東|収納

room("キッチン", 0, 0, 2730, 2730, 2, texture="wood_oak")
room("トイレ", 2730, 0, 910, 1820, 2, texture="tile_floor")
room("パントリー", 2730, 1820, 910, 910, 2, texture="wood_oak")
room("廊下", 3640, 0, 910, 3640, 2, texture="wood_oak")
room("階段", 4550, 0, 910, 3640, 2, texture="wood_oak")
room("ダイニング", 0, 2730, 3640, 2730, 2, texture="wood_oak")
room("LDK", 3640, 3640, 1820, 1820, 2, texture="wood_oak")
# 全幅5.46m、奥行2.73mの14.91㎡。ソファの真上を開け、高窓の光を奥まで届ける。
room("リビング吹抜", 0, 5460, 5460, 2730, 2, texture="wood_oak",
...
door("door-opening", 1365, 2730, 2730, 2)                      # キッチン↔LDK 全面開口
door("door-opening", 3640, 3185, 780, 2, vertical=True)       # 廊下↔LDK
      # LDK↔LDK東 大開口
                      # 収納(LDK東から)

# 窓はシンクの真上だけ。天板(850)より上に立ち上げ、東のレンジフードと離す
dress(win(792, 0, 2, "07409", 900, 1000), "roller")                    # キッチン北(シンク上)
...
# 個室を北から連続させ、横断廊下を無くして南を大きく開ける。
wall(3640, 0, 3640, 5460, 3)
wall(4550, 910, 4550, 3640, 3)
wall(4550, 3640, 5460, 3640, 3, wallStyle="solid", wallHeight=1100)
wall(0, 2730, 1820, 2730, 3)
wall(1820, 2730, 3640, 2730, 3)
wall(0, 5460, 2730, 5460, 3)
wall(2730, 5460, 3640, 5460, 3)
wall(3640, 5460, 5460, 5460, 3, wallStyle="solid", wallHeight=1100)
room("洋室A", 0, 0, 3640, 2730, 3, texture="wood_oak")
room("洋室B", 0, 2730, 3640, 2730, 3, texture="wood_oak")
room("ホール", 3640, 0, 910, 3640, 3, texture="wood_oak")
room("ギャラリー", 3640, 3640, 1820, 1820, 3, texture="wood_oak")
door("door-swing", 3640, 1450, 780, 3, vertical=True, flipY=True)
door("door-swing", 3640, 4180, 780, 3, vertical=True, flipY=True)
dress(win(2040, 0, 3, "11909", 660, 1370), "roller")
dress(win(0, 1365, 3, "07409", 1200, 830, vertical=True), "roller")
win(BW, 2275, 3, "F03613", 660, 1370, vertical=True, kind="fix")
dress(win(0, 3800, 3, "07409", 1200, 830, vertical=True), "roller")
win(0, 5050, 3, "03613", 1200, 830, vertical=True, kind="casement")
# 吹抜は南と東の二方向採光。個室を介さず光を取り込む。
win(1820, BD, 3, "16513", 400, 1630, kind="fix")
win(BW, 6650, 3, "16513", 400, 1630, vertical=True, kind="fix")
...
AC_PAIRS = [
    (190, 6300, 90, 1, 2150, -330, 6600, -90),   # 主寝室の足元側へ送風
    # (室内機 cx, cy, rot, floor, elev, 室外機 cx, cy, rot)
    (190, 5500, 90, 2, 2050, -330, 5600, -90),   # LDK(西外壁・窓より南)
    (800, 2925, 180, 3, 2050, -330, 4400, -90),  # 主寝室(廊下側の壁)
...
# ── ランドリー。物干しと作業カウンターを同じ場所へ。
ceiling_mounted("original-laundry-rail", 910, 2800, 1, w=1100, d=80)
item("im0261-Cabinet-MEGA_PACK_CABINET-cabinet-19565_Frame",
     500, 2030, 750, 300, 1, rot=180, elev=1600)
item("im0261-Cabinet-MEGA_PACK_CABINET-cabinet-19565_Frame",
     1290, 2030, 750, 300, 1, rot=180, elev=1600)
item("original-laundry-cabinet", 910, 2130, 900, 500, 1, rot=180)
...
item("original-bed", 1150, 4800, 1600, 2150, 1, rot=180,
...
# 外出用コート・バッグは玄関を上がったホール東壁へ。
item("fmp-Closet01", 5013, 4550, 1523, 773, 1, rot=-90)
# 主寝室から使う奥行910のリーチイン収納。
item("fmp-Closet01", 3190, 7050, 1523, 773, 1, rot=-90)
...
for t, w in (("fmp-CabinetD01", 366), ("fmp-CabinetD_Sink", 732),
             ("fmp-CabinetD01", 366), ("fmp-CabinetD01", 366),
             ("fmp-CabinetD02", 366), ("fmp-CabinetD03", 364)):
    item(t, kx + w / 2, 320, w, 519, 2, rot=180)
    kx += w
item("fmp-GasStove07", 2254, 320, 572, 507, 2, rot=180, elev=797)
item("fmp-KitchenExhaust07", 2254, 320, 466, 466, 2, rot=180, elev=1970)
...
item("im0261-Kitchen-MEGA_PACK_kitchen-electronic-298603_Frame_Black",
     250, 320, 340, 351, 2, rot=180, elev=797)
...
item("im0261-Tableset-MEGA_PACK_Tableset-tableset_614454_Frame_Walnut",
     1820, 4100, 1758, 1329, 2, rot=0)

# ── リビング。テレビは西壁、ソファはその正面 [check39]
# 東側(x2790-3580)は上から下まで空けておく。ここが廊下→LDK東→吹き抜けの通り道
item("original-sideboard",
     280, 6250, 1400, 420, 2, rot=90)
item("im0261-Tv-MEGA_PACK_tv-electronic-280915", 260, 6250, 1230, 211, 2,
     rot=90, elev=760)
item("im0261-Carpet-MEGA_PACK_Carpet-carpet-224774_frame_gray",
     1400, 6250, 2000, 1500, 2, rot=90)
item("original-sofa", 3000, 6050, 2100, 900, 2, rot=-90, finishColors={"fabric":"#C4BDAF", "wood":"#705842"})
item("fmp-Table36", 1850, 6100, 633, 635, 2, rot=90)
item("im0261-Decor-MEGA_PACK_decor-decor-roland_pom_pom_chrysanthemum_flower_frame_li",
     1850, 6100, 270, 209, 2, rot=90, elev=357)
# 吹き抜けの下(y6370-8190)は光の間。床には何も置かない。緑はLDK東の窓辺へ
item("im0261-Plant-MEGA_PACK_Plant-plant-230510", 5050, 4300, 618, 719, 2, rot=0)

# 家族の書類・ゲーム用品は南の低い収納へ。視線を遮らず、読書机からも使える。
item("original-sideboard", 4200, 7920, 1400, 420, 2, rot=0)
# 読書・在宅作業は東壁へ。ソファ背後と椅子の間に通路を残す。
item("im0261-Table-MEGA_PACK_Table-table-175980_frame_brown", 5100, 6200, 1758, 600, 2, rot=-90)
item("fmp-Chair29", 4450, 6200, 430, 442, 2, rot=90)
item("im0261-Lamp-MEGA_PACK_lamp-lamp-25416", 5100, 6000, 161, 273, 2, rot=-90, elev=739)

# 吹き抜けのシーリングファン。天井5.2mの空気を回す
ceiling_mounted("fmp-CeilingFan01", 1820, 7280, 2, w=1200, d=1200, rot=0)

# ══════════════════════════ 3F 家具 ══════════════════════════
# 同じ広さの子ども室。ベッド・机・衣類収納をそれぞれ確保。
for fl, by in [(3, 0), (3, 2730)]:
    item("fmp-Bed05", 850, by+1150, 1112, 1950, fl, rot=180)
    item("fmp-Table44", 2800, by+340, 1049, 524, fl, rot=180)
    item("fmp-Chair29", 2800, by+950, 430, 442, fl, rot=0)
    item("im0261-Lamp-MEGA_PACK_lamp-lamp-573754_frame", 3100, by+340, 245, 239, fl,
         rot=180, elev=626)
    item("original-wardrobe", 2000, by+2380, 1000, 580, fl, rot=0)
    item("original-wardrobe", 3000, by+2380, 1000, 580, fl, rot=0)
    item("im0261-Carpet-MEGA_PACK_Carpet-carpet-224774_frame_gray",
         2200, by+1700, 2000, 1500, fl)
# 3階ギャラリーは衣類収納と兼用。西側に有効約980mmの通行幅。
item("fmp-Closet01", 5013, 4450, 1523, 773, 3, rot=-90)
...
light("down", 1420, 4100, 2, shadow=True)
light("down", 2240, 4100, 2)   # ダイニング(食卓の中心)
...
light("ceiling", 1365, 4100, 3, shadow=True)   # 主寝室
light("down", 3185, 4100, 3)                   # 洋室B収納前
light("down", 4095, 4450, 3)                # 洋室B


# ══════════════════════════ 注記 ══════════════════════════
item("memo", 1200, -1600, 2200, 500, 1, color="#fff3a6",
     noteText="都市型3階建てモデルプラン 3LDK・大吹抜\n"
              "敷地約83㎡ / 建物外形5.46×8.19m\n"
...
plan = dict(
    walls=P.walls, rooms=P.rooms, items=P.items,
    floorMetadata={
...
# Preserve the accepted user edits, then resolve the remaining physical conflicts.
from default_plan_review import apply_review
...
# 最新の受領版(review 24)。水まわりの寄せ、外構の作り直し、木部テクスチャの
# 指定。差分は tools/make_review_patch.py で作る。
apply_patch_file(plan, "default_plan_3f_review_24.json")
_review24 = _json.loads(_Path(__file__).with_name("default_plan_3f_review_24.json").read_text())
plan.update(_review24["metadata"])
