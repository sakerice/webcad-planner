...
別の未解決の色付けの癖であり、この検証コードの守備範囲外だった。旧実装は
この癖のせいで「家具が揃っている run」でも「家具が全く無い run」でも
ほぼ同じ 0〜数 % しか報告できず、実質的に何も見分けていなかった。

その黒塗り自体は後に index.html 側で直っている (captureInstance3DData が
guide マテリアルに vertexColors を立てており、COLOR_0 を持たない GLB
ジオメトリでは未束縛属性の既定値 (0,0,0,1) が instance の色を 0 倍して
いた)。実測: 修正後の T92 相当のフレームで黒画素 0.0000%、legend 73 件中
63 件が発色 (欠けた 10 件は真俯瞰で見えない窓・机下の椅子で、斜俯瞰から
撮り直すとその 7 件が現れる)。下の面積ベースの判定は色付けが直った後も
そのまま成立する (家具の宣言色は "非家具色集合" に入らないため) ので、
このチェックは黒塗り前提のままにはしていない。
